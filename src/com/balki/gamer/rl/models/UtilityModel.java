 ...
 * @since 21/12/2018
 *
 *        Utility value of a state $U(s)$ is the expected long term reward of
 *        state $s$ given the sequence of reward and the optimal policy Utility
 *        value $U(s)$ at state $s$ can be obtained by the Bellman equation
 *        Bellman Equtation states that $U(s) = R(s) + \gamma * max_a \sum_{s'}
 *        T(s,a,s')U(s')$ where s' is the possible transitioned state given that
 *        action $a$ is applied at state $s$ where $T(s,a,s')$ is the transition
 *        probability of $s \rightarrow s'$ given that action $a$ is applied at
 *        state $s$ where $\sum_{s'} T(s,a,s')U(s')$ is the expected long term
 *        reward given that action $a$ is applied at state $s$ where $max_a
 *        \sum_{s'} T(s,a,s')U(s')$ is the maximum expected long term reward
 *        given that the chosen optimal action $a$ is applied at state $s$
 */
public class UtilityModel implements Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = 4884919108437078647L;
